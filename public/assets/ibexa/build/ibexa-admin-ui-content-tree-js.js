...
var ContentTreeModule = function ContentTreeModule(props) {
    ...
    return findLocationsByIdWrapper({
      ...
      if (notDeletedItems.length) {
        ...
        if (anyUserContentItemDeleted && anyNonUserContentItemDeleted) {
          ...
          _message3 = Translator.trans( /*@Desc("User(s) deleted.")*/'bulk_delete.success.message.users', {}, 'sub_items');
        } else {
  ...
  var loadMoreSubitems = function loadMoreSubitems(item) {
    return (0,_common_services_content_tree_service__WEBPACK_IMPORTED_MODULE_15__.loadLocationItems)(_objectSpread(_objectSpread({}, props.restInfo), {}, {
      ...
      var nextItem = (0,_common_helpers_tree__WEBPACK_IMPORTED_MODULE_17__.findItem)([tree], item.internalItem.path.split('/'));
      if (nextItem) {
        ...
        var nextTreeItem = (0,_common_helpers_tree__WEBPACK_IMPORTED_MODULE_17__.findItem)([nextTree], item.internalItem.path.split('/'));
        nextTreeItem.subitems = [].concat(_toConsumableArray(nextTreeItem.subitems), _toConsumableArray(location.subitems)).map(function (subitem) {
          return _objectSpread(_objectSpread({}, subitem), {}, {
            path: "".concat(nextTreeItem.path, "/").concat(subitem.locationId)
          });
        });
        (0,_common_helpers_tree__WEBPACK_IMPORTED_MODULE_17__.updateItemInSubtree)(subtree.current[0], nextTreeItem, item.internalItem.path.split('/'));
        saveSubtree();
        setTree(nextTree);
      }
    })["catch"](window.ibexa.helpers.notification.showErrorNotification);
  ...
  var getInitiallyExpandedItems = function getInitiallyExpandedItems(items) {
    var expandedItems = items !== null && items !== void 0 ? items : [];
    var path = currentLocationPath.split('/').filter(function (id) {
      return !!id;
    });
    var rootLocationIdIndex = path.findIndex(function (element) {
    ...
    var pathStartingAfterRootLocation = path.slice(rootLocationIdIndex - path.length);
    pathStartingAfterRootLocation.forEach(function (locationId) {
      var isLocationAlreadyExpanded = expandedItems.some(function (_ref8) {
        var id = _ref8.id;
        return id === parseInt(locationId, 10);
      });
      if (!isLocationAlreadyExpanded) {
        expandedItems.push({
