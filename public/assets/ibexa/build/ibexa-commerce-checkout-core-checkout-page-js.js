"use strict";
(self["webpackChunk"] = self["webpackChunk"] || []).push([["ibexa-commerce-checkout-core-checkout-page-js"],{

/***/ "./vendor/ibexa/commerce-checkout/src/bundle/Resources/public/js/core/checkout.page.js":
/*!*********************************************************************************************!*\
  !*** ./vendor/ibexa/commerce-checkout/src/bundle/Resources/public/js/core/checkout.page.js ***!
  \*********************************************************************************************/
...
(function (global, doc) {
  ...
  var CheckoutPage = /*#__PURE__*/function () {
    function CheckoutPage() {
      ...
      this.steps = _toConsumableArray(doc.querySelectorAll('.ibexa-commerce-js--step-wrapper')).map(function (step) {
        return step.id;
      });
    ...
    _createClass(CheckoutPage, [{
      ...
      value: function clearErrors(form) {
        var errorElement = form.querySelector('.ibexa-commerce-js--checkout-error-message');
        if (errorElement) {
          errorElement.innerHTML = '';
        }
        form.querySelectorAll('.ibexa-commerce-js--field-error').forEach(function (fieldError) {
        ...
        if (formErrors) {
          ...
          Object.entries(errors).forEach(function (_ref5) {
            var _ref6 = _slicedToArray(_ref5, 2),
              path = _ref6[0],
              error = _ref6[1];
            form.querySelector("[name=\"".concat(path, "\"]")).closest('.ibexa-commerce-js--field-wrapper').querySelector('.ibexa-commerce-js--field-error').innerHTML = error.join('\n');
          });
        }
      }
    }, {
      ...
      value: function fillSummarySidebar() {
        ...
        var sidebarSummaryList = sidebarSummary.querySelector('.ibexa-commerce-sidebar-summary__list');
      ...
      value: function showForceStop() {
        ...
        if (response.force_step_hide === false) {
          doc.querySelector("#".concat(response.current_step, " .ibexa-commerce-js--force-step")).hidden = false;
        }
      }
    }, {
      ...
      value: function bindDeliveryFormEvents() {
        ...
        doc.querySelectorAll('input[name="checkout_delivery_address[addressStatus]"]').forEach(function (input) {
          return input.addEventListener('change', function (event) {
            doc.querySelector('.ibexa-commerce-existing-delivery-address').classList.remove('ibexa-commerce-existing-delivery-address--visible');
            if (event.currentTarget.value === 'sameAsInvoice') {
              _this.copyInvoiceToDelivery();
            } else if (event.currentTarget.value === 'existing') {
              doc.querySelector('.ibexa-commerce-existing-delivery-address').classList.add('ibexa-commerce-existing-delivery-address--visible');
        ...
        doc.querySelectorAll('.ibexa-commerce-js--address-button').forEach(function (button) {
          return button.addEventListener('click', function (event) {
            var address = JSON.parse(event.currentTarget.closest('.ibexa-commerce-js--address-wrapper').dataset.address);
            _this.addressKeys.forEach(function (key) {
      ...
      value: function bindShowStepEvents() {
        var _this2 = this;
        doc.querySelectorAll('.ibexa-commerce-checkout__step-action--show').forEach(function (button) {
          return button.addEventListener('click', function (event) {
            _this2.validateState(event.currentTarget.closest('.ibexa-commerce-js--step-wrapper').id);
          });
        });
      }
    }, {
      ...
      value: function bindSubmitEvents() {
        ...
        doc.querySelector('.ibexa-commerce-js--button-buy-as-guest').addEventListener('click', function (event) {
          event.preventDefault();
      ...
      value: function validateState(customCurrentStep) {
        ...
        var validateUrl = doc.querySelector('.ibexa-commerce-js--checkout-wrapper').dataset.validateUrl;
        if (currentStep) {
...
/******/ var __webpack_exports__ = (__webpack_exec__("./vendor/ibexa/commerce-checkout/src/bundle/Resources/public/js/core/checkout.page.js"));
/******/ }
]);